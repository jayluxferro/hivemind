...
def fig_failure_rates():
    ...
    bars1 = ax.bar(x - width / 2, direct_rates, width, label="Direct", color="#d62728", alpha=0.85)
    bars2 = ax.bar(x + width / 2, hm_rates, width, label="HiveMind", color="#2ca02c", alpha=0.85)
...
def fig_throughput():
    ...
    ax1.plot(agents, direct_completed, "o-", color="#d62728", label="Direct", markersize=4, linewidth=1.5)
    ax1.plot(agents, hm_completed, "s-", color="#2ca02c", label="HiveMind", markersize=4, linewidth=1.5)
    ...
    # Right: throughput
    ax2.plot(agents, direct_tp, "o-", color="#d62728", label="Direct", markersize=4, linewidth=1.5)
    ax2.plot(agents, hm_tp, "s-", color="#2ca02c", label="HiveMind", markersize=4, linewidth=1.5)
...
def fig_ablation():
    ...
    for bar, rate in zip(bars, fail_rates):
        if rate > 0:
            ax.text(bar.get_width() + 1, bar.get_y() + bar.get_height() / 2, f"{rate:.1f}%", va="center", fontsize=7)
...
def fig_token_waste():
    ...
    fig, ax = plt.subplots(figsize=(4.5, 2.2))
    ax.bar(x - width / 2, [w / 1000 for w in direct_waste], width, label="Direct", color="#d62728", alpha=0.85)
    ax.bar(x + width / 2, [w / 1000 for w in hm_waste], width, label="HiveMind", color="#2ca02c", alpha=0.85)
...
def fig_cost_analysis():
    ...
    total_waste_direct = sum(
        c["direct"]["wasted_tokens"] for i, c in enumerate(data["comparisons"]) if i < 7
    )  # first 7 unique
    total_waste_hm = sum(c["hivemind"]["wasted_tokens"] for i, c in enumerate(data["comparisons"]) if i < 7)
    ...
    ax.bar(x - width / 2, direct_costs, width, label="Direct", color="#d62728", alpha=0.85)
    ax.bar(x + width / 2, hm_costs, width, label="HiveMind", color="#2ca02c", alpha=0.85)
